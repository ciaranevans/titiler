"""Builder that produces a zip in the current workspace from the Titiler docker image"""
import io
import os
import tarfile
...
class TitilerLambdaBuilder:
    ...
    def __init__(self):
        """Initialising the TitilerLambdaBuilder"""
        current_dir = os.path.dirname(os.path.realpath(__file__))
        self.titiler_lambda_package_filepath = os.path.join(
            current_dir, "titiler_lambda.zip"
        )
    ...
    def _get_container():
        ...
        client.images.build(path="./lambda/titiler", tag="lambda:latest")
        return client.containers.run(
            image="lambda:latest", command="/bin/bash", detach=True
        )
    ...
    def get_package_path(self):
        """Returns path to a zipfile which was built using the Titiler docker image"""
